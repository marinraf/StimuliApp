...
class EditListOfValuesMenu: Menu {
    ...
    private func makeSection4() {
        ...
        makeSequenceValue(sectionNumber: sectionNumber)
        makeLogSequenceValue(sectionNumber: sectionNumber)
    }
    ...
    func makeOptionFloat(from property: Property, sectionNumber: Int) {
        ...
        option.nextScreen = {
            ...
            let modify = Modify(title: "value",
                                info: """
                                You can introduce multiple values at once separated by commas.
                                """,
                                property: property)
            modify.saveFunctionString = { response in
                ...
                for element in responses {
                    guard let float = Float(element) else { return .saved }
                    newProperties.append(Property(name: "",
                                                  info: "",
                ...
                Flow.shared.listOfValues.values.remove(at: property.listOrder)
                for index in 0 ..< newProperties.count {
                    Flow.shared.listOfValues.values.insert(newProperties[index], at: property.listOrder + index)
                    let newProperty = Property(name: "",
                                               info: "",
                                               propertyType: .simpleFloat,
                                               unitType: .decimal,
                                               float: 0)
                    for section in self.getSections() where index > 0 {
                        if section.trialValue.properties.count > 0 {
                            if let valueType = FixedValueType(rawValue: section.trialValue.properties[0].string) {
                                if valueType == .other {
                                    section.trialValue.properties[0].properties.insert(newProperty,
                                                                                       at: property.listOrder + index)
                                }
                            }
                        }
                    }
                ...
                for (index, element) in Flow.shared.listOfValues.values.enumerated() {
    ...
    private func makeNewValue(sectionNumber: Int) {
        var option = Option(name: "+ add value(s)")
        option.style = .insert
        let info = """
        Add a new value to the list. You can introduce multiple values at once separated by commas.
        """
    ...
    private func makeSequenceValue(sectionNumber: Int) {
        var option = Option(name: "+ add linear sequence")
        option.style = .insert
        option.infoMessage = """
        Create a linear sequence of values by introducing the first value, the last value \
        ...
        option.nextScreen = {
            ...
            let modify = Modify(title: "linear sequence",
                                info: "First value - Last value - Number of values",
                                property: property)

            modify.saveFunctionFloats = { response in
                property.changeValue(new: response)
                let newValues = AppUtility.createSequenceArray(firstValue: property.float,
                                                               lastValue: property.float1,
                                                               numberOfValues: property.float2)
                for (index, float) in newValues.enumerated() {
                    let newValue = Property(name: "",
                                            info: "",
                                            propertyType: .simpleFloat,
                                            unitType: .variableUnit,
                                            float: float)
                    newValue.listOrder = Flow.shared.listOfValues.values.count
                    Flow.shared.listOfValues.values.append(newValue)
    ...
    private func makeLogSequenceValue(sectionNumber: Int) {
        var option = Option(name: "+ add logarithmic sequence")
        option.style = .insert
        option.infoMessage = """
        Create a logarithmic sequence of values by introducing the first value, the last value \
        and the total number of values. (First value and last value must be both > 0 or both < 0)
        """
        option.nextScreen = {
            let property = Property(name: "",
                                    info: "",
            ...
            let modify = Modify(title: "logarithmic sequence",
                                info: "First value - Last value - Number of values",
                                property: property)
            modify.saveFunctionFloats = { response in
                property.changeValue(new: response)
                let newValues = AppUtility.createLogSequenceArray(firstValue: property.float,
                                                                  lastValue: property.float1,
                                                                  numberOfValues: property.float2)
                for (index, float) in newValues.enumerated() {
                    let newValue = Property(name: "",
