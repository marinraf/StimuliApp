...
class EditListOfValuesAudio: Menu {

    // MARK: - Setting
    override func setting() {
        title2 = Flow.shared.listOfValues.name.string
        backButton = "< Lists of audios"
    ...
    private func makeNewValue(sectionNumber: Int) {
        var option = Option(name: "+ add audio")
        option.style = .insert
        let info = """
        Add a new audio to the list.
        The audio will be imported from the music library.
        You won't be able to import files with DRM restrictions.
        """
