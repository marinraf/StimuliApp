...
struct Texts {
    ...
    It is possible to make an audio play for its entire duration, meaning, it does not stop every time we change scenes \
    but continues to play through different scenes and trials.
    This can be useful for playing an audio or background \
    music throughout the duration of a test. For example, an audio can be generated containing a whole sequence of sounds \
    and used to measure mismatch negativity while a test is being conducted.
    """
    
    static let distanceMeasure = """
    Activate the subject-to-screen distance measurements for this test. Requires the use of the frontal camera.
    """
    ...
    static let calibration = """
    First, we are going to calibrate the eye tracker.
    Press the button to start the calibration. Look at the point that appears on the screen until it disappears.
    Repeat the process until the calibration is done.
    If the results are good, you can continue to the test. Otherwise, you can try to calibrate again.
    """
    
    static let calibrationError = """
    Eye tracker error. Please verify that your device has an active internet connection.
    """
    ...
    If the distance between the point the user is looking at and the center of the screen is greater than this value, \
    the tracking error is triggered.
    """
    ...
    static let gazeFixation = """
    The user must look at the center of the screen. Otherwise, a warning message will appear and the trial will be skipped.
    """
